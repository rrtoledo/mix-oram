...
\end{tikzpicture}
\centering
\caption{Cascade Mix-ORAM.\newline Rebuild method (all phases) and Layered method (only the Wrapping phase) } \label{fig:eviction}
\end{figure} 
...
The mixes have each been allocated a chunk of the database ($mix_{idx}$ having $[idx\cdot n/m : (idx+1)\cdot n/m]$) and use the public permutation seeds $\sigma_{pub}$ to compute which record to send to each.
...
\end{minipage}
\centering
\caption{Parallel Mix-ORAM with 3 mixes.\newline Rebuild method (all phases) and Layered method (only the Wrapping phase).}\label{fig:Par}
\end{figure*}
