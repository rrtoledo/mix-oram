...
Shuffle and sorting algorithms are a thouroughly researched subject central to ORAM for the randomization process. However most of them are not oblivious in that the memory accesses done by algorithm depends on the input.
...
\subsection{Model}
We consider on one side, an ORAM remote server, defined as follows, with memory capacity of $\mathcal{O}\left(n\right)$ $b$-bit long blocks, a mixnet composed of $m$ mixes and on the other side, a client with memory of $s$-block capacity with $s<<n$.
...
The adversary might have different goals such as identifying a user data access or matching encrypted datablocks with known plaintext for further purposes such as selective data corruption or monitoring.
...
In addition we assume adversary may have corrupted the ORAM server and a part of the mixnet to achieve his goal.
We will assume until the discussion\cite{Discussion} that the compromised machines present a \textit{honest but curious} behaviour in that every operation is correctly performed but passively recorded until the discussion section (c.f. Sec.~\cite{Discussion}) where we will examine packet injection, dropping and delaying.

This work focuses on the ORAM eviction process where sequences of datablocks are merged in an oblivious manner in order to hide the records indexes after access information has leaked. For instance, in the Square Root solution the problem refers to the eviction of the shelter in the database and the upper partitions in a lower one in the Partition cases.
...
A pseudorandom generator (PRG) is a deterministic function that produces pseudorandom strings, it can be instanticiated with a small size parameters, the seeds, to produce the same sequence of characters on any machine provided the latters have functions with the same versions.
...
In the case of classic client-server configuration, the client has stored the previous seed and the new one and has to permute obviously the records such that the new ordering is not revealed. This can be done easily thanks to the user storage: the user can download the whole database, sort it thanks to the indexes, reencrypt it and upload it back.
With the introduction of the mixnet, we have a shift of paradigm: the permutations are now executed by the untrusted mixnet. Thereby, seeds are provided to each mix without any information about the indexes so that the adversaries only have a partial view of the permutation process.
For the user to remember as little information as possible, we could find seed combinations resulting in a simple result. However, because the symmetric group is not commutative and the seeds space isn't structured, finding such combinations can only be done through heavy computation.
...
ORAM designs heavily rely on encryption mechanism to obfuscate the database records. Every record that is fetched must be reencrypted before uploading it back to the database 
...
Futhermore, AES in Counter mode (AES-CTR) encryption and decryption operations are commutable and commmutative. AES-CTR can thus help in the eviction process with reencrypting with a new layer the datablocks before decrypting the former one such that the plaintext is never revealed.
...
We now consider the mixnet as a collection of isolated mixes. When performing an eviction, we attribute to each mix  $k=\frac{n}{m}$ distinct indexes and send them a list of $\mathcal{O}\left ( m \cdot \log \left (\frac{n}{s} \right ) \right )$ of private and public seeds and encryption keys.
The unwrapping and wrapping phases consist of $\mathcal{O}\left ( m \cdot \log \left (\frac{n}{s} \right ) \right )$ rounds. In the unwrapping phase, the records are located in the database with the old public seeds, inversely permuted with the old private seeds and reencrypted with the new encryption key. 
In the wrapping phase, the records are permuted according to the new seeds, decrypted with the old encryption keys and uploaded to the database according to the new public seeds as shown in Fig.~\ref{fig:WAS}.

\begin{algorithm}
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Seeds $old\_S_{pub,j},\ old\_S_{prv,i,j}$; \\ \qquad Encryption key $new\_key_{i,j}$; \\ \qquad Number of records $k$; \\ \qquad First index $index$;}
$Idxs \gets \{{\Pi}_{old\_S_{pub,j}}^{-1} \left ( range(index, index +k)\right ) \}$;\\
$Records \gets \{\}$;\\
$i\gets 0$;\\
\While{$i < k$}
{
	$Records.append\left (get(Idxs[i])\right)$;\\
	$i\gets i+1$;\\
}
$Records \gets \{ {\Pi}_{old\_S_{priv,i,j}}^{-1} \left ( Records \right ) \} $;\\
\ForAll{$record\in Records$}
{
	$record \gets encrypt_{new\_key_{i,j}}\left( record \right )$;
}
$put\_range \left(index,index+k, Records \right )$;\\
$\textbf{del } old\_S_{pub,j},\ old\_S_{priv,i,j}$;\\
\caption{Unwrapping process for the $mix_i$ during $round_j$}
\end{algorithm}

\begin{algorithm}
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Seeds $new\_S_{pub,j},\ new\_S_{prv,i,j}$; \\ \qquad Encryption key $old\_key_{i,j}$; \\ \qquad Number of records $k$; \\ \qquad First index $index$;}
$Records \gets get\_range\left (idx,\ idx+k\right)$;\\
$Records \gets \{ {\Pi}_{new\_S_{priv,i,j}} \left ( Records \right ) \} $;\\
\ForAll{$record\in Records$}
{
	$record \gets decrypt_{old\_key_{i,j}}\left( record \right )$;
}
$Idxs \gets \{{\Pi}_{new\_S_{pub,j}} \left ( range(index, index +k)\right ) \}$;\\
$i\gets 0$;\\
\While{$i < k$}
{
	$put\left( Idxs[i],\ Records[i] \right )$;\\
	$i\gets i+1$;
}
$\textbf{del } old\_key_{i,j}$;\\
\caption{Wrapping process for the $mix_i$ during $round_j$}
\end{algorithm}

\todo{Explanation of figure, message format}

\begin{figure}
\centering
\begin{adjustbox}{width=\textwidth/3, height=\textwidth/3}
\begin{tikzpicture}[->,
auto,
semithick, 
scale=0.5,
...
\node[draw=none, fill=none]    		(X)[above of=B, yshift=-4em]		  {...};
\node[draw=none, fill=none]    		(Y)[below of=B, yshift=4em]		  {...};
\node[block, anchor= north]		(A)[above of=X, yshift=1em]		{$mix_1$};
\node[block, anchor = south]		(C)[below of=Y, yshift=-1em]		{$mix_m$};
...
\path[->, midway]
 ...
 (A.east) edge node			{} 			([yshift=2ex]1.south west)
 (B.east) edge node			{\small $S_{pub}$} 	([yshift=-2ex]1.north west)
 (C.east) edge node			{} 			([yshift=8ex]1.south west)
 (C.east) edge node			{} 			(1);
 
\end{tikzpicture}
\end{adjustbox}
\caption{Wraping in a round of Amortizable sort} \label{fig:WAS}
\end{figure} 
...
\begin{itemize}
 ...
 \item Differentially private oblivious shuffle
\end{itemize}
