...
The scheme relies on the commutativity of the xor operation used in AES-CTR, and on impossibility to decrypt the tags beforehand thanks to the threshold encryption scheme. 
%
...
\noindent\textbf{Costs.}
We have $c_{cmp} \approx 2\cdot m \cdot c_{CBC}$, $c_{client}= x$, $c_{oram}=n$, $c_mix=n$. The total communication cost is $c_{com}=2\cdot m\cdot n + m\cdot c_{client}$.\\
...
\node[draw, rectangle, minimum height=3cm]    		(T)[right =of b]   {\small $blind$};
