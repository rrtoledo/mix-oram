...
\noindent\textbf{Renovation method.} The renovation method consists in replacing all the mix encryption layer with new ones such that the client encryption remains oblivious, we do so with AES in CTR mode. Contrary to the Layered method, the renovation method does not rely on a data structure, and the encryption depends on the records order. We use the CTR mode to make use of its inherent commutativity properties.
