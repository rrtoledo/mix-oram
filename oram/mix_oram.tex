...
We make the assumption that all data has first been encrypted with the client private keys  before using the ORAM. \\
...
\node[draw, rectangle, align=center, label=below:{$D\left ( k  \right )$}]   		(A)[right of=X]   {$\Pi_{\sigma}^{-1}$};
...
\node[draw, rectangle, align=center, label=above:{$E\left ( k'  \right )$}]    		(C)[right of=B3]   {$\Pi_{\sigma'}$};
\node[draw, cylinder, shape border rotate=90, minimum height=3em,minimum width=2em]    	(Y)[right of=C]	  {DB};
\node[draw=none, align=center] (YY)[below of=Y] {\small \textbf{Status}\\$E_{k0'}, (\Pi_{\sigma'}, E_{k1'})$};
\draw[decoration={brace,mirror, amplitude=1em}, decorate]  ([yshift=-2em]A.south east) -- ([yshift=-2em]C.south west) node[below, pos=0.5, yshift=-1em] {Simple E/D phase};
\path[->]
...
\end{tikzpicture}
\centering
\caption{Eviction under the Rebuild method, with encryptions (E) and decryptions (D) and permutations ($\Pi$).} \label{fig:AES}
\end{figure*}
