...
\DontPrintSemicolon
\KwIn{Shared key $Key_{i,j}$; \\ \qquad Collection of data blocks $d$;}
\ForAll{$r \in d$}
{
	$IV \gets urandom(d[r].token, 128)$;\\
	$d[r].data \gets cbc\left(Key_{i,j},\ IV,\ d[r].data \right )$;\\
	$IV \gets urandom(d[r].data, 128)$;\\
	$d[r].token \gets cbc\left(Key_{i,j},\ IV,\ d[r].token \right )$;\\
}
\KwOut{data}
\caption{Layered algorithm for mix $i$ at round $j$}
\label{alg:pile}
\end{algorithm}
...
\If{$record == null$}
{
$dummies \gets compute\_dummies(cache, d+1)$;\\
}
\Else
{
$dummies \gets compute\_dummies(cache, d, j)$;\\
}
