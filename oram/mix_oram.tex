...
\noindent\textbf{Layered method.} In the Layered method, we use the data structure shown in Table~\ref{ldata} composed of an IV token, and a label appended to the record, e.g. the record local indices. The underlying principle of the layered method is to let the number of encryption and permutation layers grow. The records are encrypted and permuted by the mixes during the eviction, and only decrypted by the client during the access. We moreover assume the existence of a record index the client can access, e.g to link the records' names to their position in the local database, and make use of it to store the current remote indices. % The index takes up $n\cdot log(n)$ bits, 50 MB can store more that $10^8$ indices. 
Before sending the records to the ORAM database, the client first needs to encrypt and permute them. Each record is thus appended with its local index and a random IV token, the tokens and data are then encrypted with the client secret key according to Algorithm~\ref{lencp} and permuted with a random seed, the index is then updated with the new remote indices.\\
...
When retrieving a record, the client needs to decipher all encryption layers. As the number of encryption layers per record varies, a timing attack can occur : the adversary can guess when a record was last fetched. To prevent the attack, we modify the access method so that the decryption happens offline (c.f. Algorithm~\ref{alg:lacc}). After retrieving a record (line 1 to 9), the client now directly encrypts it with its own key and updates it in the cache stored locally (line 10). The local cache is then uploaded to the remote server (line 11).
After doing so, if it is a write operation, the client overwrite the record with its new version (line 12 to 13). If it is a read operation, the client locally decrypts the previous encryption and remove the mix encryption layers (line 14 to 15). And finally, the client save locally an encrypted version of the record (line 16).\\ 

\begin{algorithm}
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{ Local record index $index$; \\ \qquad Operation and data $op,\ towrite$; \\ \qquad Encryption keys $k$; \\ \qquad Permutation seeds $\sigma$;\\ \qquad Saved records $recs$;\\ \qquad Number of rounds r;}
...
To decrypt a record, the client uses a trial error (c.f. Algorithm~\ref{alg:ldec}) based on the primitive~\ref{alg:ldecp} where, using the shared secrets, the IV token is first deciphered then the data. The decryption algorithm first removes the client encryption, and executes the primitive $r$ times. The client then uses its own secret key to decipher the IV and the data another time, if the label is the right one, the process stops, if not the client cancels the last decryption and starts again.\\
...
$j,e=0$;\\
$r\gets \textbf{decrypt\_prm}(prv_e,rec)$;\\
...
\ForAll{$k \in \llbracket 1 :r \rrbracket $}
{
	...
	$rec \gets \textbf{decrypt\_prm}(k_{m,\ e,\ j}, rec)$;\\
	$j\gets j-1$;\\
}
	$rec \gets \textbf{decrypt\_prm}(prv_e,\ rec)$;\\
    $e\gets e-1$;\\
}
\KwOut{rec}
...
\noindent\textbf{Rebuild method.} The rebuild method aims at replacing all the mix encryption and permutation layers with new ones. The difficulty of this method is to replace the layers in a manner such that the intermediaries never see the underlying client encryption to prevent an adversary from observing which record was accessed.
...
Before sending the records to the untrusted storage, the client encrypts the plain-text thrice. The first time the record are encrypted with the client own private keys and fixed counters, the second time with keys generated from the secrets shared with the mixes and fixed counters, and finally at the last time the records are permuted and encrypted at the same time, with the permutation seeds and encryption keys generated from the shared secrets.
