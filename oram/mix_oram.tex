...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Group elements $\alpha_i,\ \beta_i$;\\ \qquad List of mixes $(ips,\ ports)$;\\ \qquad Data $data$;\\ \qquad Private key $x_i$}
$s\sigma_{i,old} = \alpha_{i,old}^{x_i}$;\\
$s\sigma_{i, new} = \alpha_{i, new}^{x_i}$;\\
$sk_{i,} = \beta_{i}^{x_i}$;\\
$k_i=hkdf(sk_i)$;\\
$\sigma_{i,old}=hkdf(s\sigma_{i,old})$;\\
$\sigma_{i,new}=hkdf(s\sigma_{i,new})$;\\
\ForAll {$d \in data$}
{
	$d \gets encrypt_cbc(k_i, d)$;\\
}
\KwOut{$mixes,\ indices$}
...
The live computation cost is the encryption and shuffle, that is to say $4\cdot m \cdot n \cdot c_{CBC}$, as the keys and seeds can be derived while the database is being received. \todo{}.\\
