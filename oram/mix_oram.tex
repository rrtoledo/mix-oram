...
%
We now consider the mix-net as a collection of isolated mixes. The unwrapping (for the rebuild method) and wrapping phases consist each of $r$ rounds.\\


\noindent\textbf{Layered method.} In that scenario, there is only one wrapping phase where the mixes keep encrypting and permuting the records. As the permutation is not undone at any moment, the assumption that the records are indistinguishable holds, the wrapping phase can then consist of $r= m/2 \cdot \log(n/s)$ rounds. \\
