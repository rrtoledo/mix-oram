...
\affil[2]{University College London (g.danezis@ucl.ac.uk)}
  ...
  \begin{abstract}
{
Oblivious RAM is a key technology for securely storing data on untrusted storage but is commonly considered impractical due to its high overhead. Indeed, to ensure full privacy to the user, the database has to be periodically randomized, i.e. re-encrypted and shuffled, by the client. The computation and communication costs the client incurs are in reality super linear in the number of records which deters the use of ORAM for most  applications. We propose in this paper to increase ORAM appeal by delegating the randomization process to semi-trusted third parties and presenting four different designs.\todo{add results}
}
\end{abstract}
...
These, however, do not prevent the leakage of meta data: the storage server owner can watch which records were fetched. 

Oblivious RAM (ORAM)~\cite{goldreich87}, or Oblivious Storage (OS)~\cite{boneh2011}, precisely prevents an adversary from observing the record access by introducing a coupling of virtual and real, or remote and local, indices. In these schemes, the remote records are permuted and information making the matching between remote and local indices, a pseudo random generator seed for instance, is saved locally. When the user requests a given record, the client computes from the seed its remote index and fetch the corresponding block. After a number of accesses, the database is then re-encrypted and shuffled during a so called eviction process.\\ 
This randomization is the main bottleneck of ORAM. Indeed, we assume in such design that the number of records stored remotely is orders of magnitude higher than the client storage. As every record must be re-encrypted and permuted locally, the client has to download and process the database by chunks, and do so several times in order to undoubtedly hide from the adversary the record ordering. Thus as the the database grows,  the randomization cost super linearly. That is why we propose here to delegate the eviction to semi trusted third parties.\\
Mix networks~\cite{chaum1981untraceable} is a solution widely applied in anonymity system to obliviously shuffle packets. A batch of packets is processed by the mix-net in that each mix re-encrypts, or decrypts, and shuffles the packets before sending them to the next mix. For the adversary to link the mix-net input and output, all of the mixes used in the shuffling have to be compromised.
...
\noindent\textbf{ORAM.}
ORAM technologies were first presented by Goldreich and Ostrovsky in 1990~\cite{ostrovsky1990efficient} to prevent reverse engineering and protect softwares run on shielded CPU, Onlivious Storage (OS)~\cite{boneh2011} in 2011 refreshed them in the context of untrusted remote storage.
...
Shuffle and sorting algorithms are a thoroughly researched subject central to ORAM for the randomization process. However most of them are not oblivious in that the permutations done depends on the data itself.
...
This work is inspired by the mix-net technology and its encryption and permutation functionalities. However, solely packet unlinkability is of interest, the anonymity property disappears as packets come from a single user at a time.
%
\section{Preliminaries}\label{Prelim}
%
\subsection{Model}\label{Model}

An ORAM system actually is composed of two types of data arrays. One that we call database and consists of the user encrypted records and dummies and one that we call cache, e.g. the shelter in~\ref{goldreich87}, which can fit up to $s$ records, $s$ being an ORAM parameter.
When accessing a record, the client downloads the cache and checks whether the desired record is present, if so the client fetches a dummy element from the database else the desired record, finally it encrypts and updates the desired element back in the cache.\\

After accessing $s$ records from an ORAM database, that is when the cache is full, the client needs to start the eviction process to randomize the database so that what little information could have gathered is nullified. To do so, the client first need to rebuild the database by merging obliviously the cache and the database and afterwards, start the oblivious shuffle.
The rebuild phase consists in placing obliviously records from the cache back to the ORAM database. The client begins then by downloading the cache, fetches all the previously accessed records  from the database once again and overwrite them one by one with either  an encryption of their cached version or with a dummy.
The client finally sends randomization instructions to a set of mixes. These instructions contains the list of participating mixes and the seeds used for shuffling and encrypting. When receiving the records, the mixes encrypt, shuffle and transmit them to the next mixes according to these instructions.
When the user requests a specific record, the client computes the remote index thanks to the seeds shared with the mix-net and retrieve the corresponding record. The client then decrypts the latter thanks to the shared encryption keys and update it back in the cache after encrypting it.\\

We consider an ORAM remote server with memory of $n\ b$-bit long blocks, a mix-net composed of $m$ mixes with memory greater than $n/m$, and a client with memory of $s$-block capacity with $s\ll n$. The remote server can additionally include a small memory of capacity $\mathcal{O}(m)$ to store extra information about permutation and encryption.
\iffalse
\begin{itemize}
...
We adopt here the ORAM definition from Stefanov et al.~\cite{stefanov2011towards} as written in the Privacy Definition~\ref{def:Oram}.
\noindent We assume that all communication between the different parties (client, ORAM server and mixes) may be intercepted as in the \textit{global passive adversary} assumption however only message timing, volume and size from honest parties can be known thanks to packet encryption.
In addition, the adversaries may have corrupted the ORAM server and a part of the mix-net to achieve their goal. Unless said otherwise, we assume that the adversaries have compromised the ORAM server and all but one mix.
...
\fi
%
\noindent\textbf{Costs.} We denote the communication costs by $\omega$ for the number of \emph{bits} sent by the ORAM, $\gamma$ by the client per mix, and by $\mu$ sent per mix. 
We will call $\pi$ the computation cost derived from the eviction.
%
...
Advanced Encryption Standard (AES)~\cite{daemen2013design} was conceived for high speed and low RAM requirements. It can present throughput over 700 MB/s per thread on recent CPUs such as the In%tel Core i3~\cite{mcwilliams2014hardware} which makes it the ideal choice for ORAM.\\ 
...
\DontPrintSemicolon
\KwIn{Encryption key $Key_{i,j}$; \\ \qquad Number of records per mix $k$; \\ \qquad Data $d$;}
\ForAll{$r \in \llbracket 1,\ k \rrbracket$}
{
...
\node[draw, rectangle, align=center]   		(A)[right of=X]   {\small $D_1$\\$P^{-1}$};
\node[draw, rectangle, align=center]    		(B)[right of=A]   {\small $E_2'$};
\node[draw, rectangle, align=center]    		(C)[right of=B]   {\small $D_2$};
\node[draw, rectangle, align=center]    		(D)[right of=C]   {\small $E_1'$\\$P_1'$};
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Encryption key $Key_{old,i,j},\ Key_{new,i,j}$; \\ \qquad Number of records per mix $k$; \\ \qquad Data $d$;}
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Number of records $n$; \\ \qquad Number of records per mix $k$; \\ \qquad Data $data$;}
$data \gets sort(data)$;\\
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Old private seed $S_{old,i}$; \\ \qquad New private seed $S_{new,i}$;\\ \qquad Data $data$;}
\If {in}
{
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Seeds $S_{old}$; \\ \qquad Total number of records $n$; \\ \qquad Number of mixes $m$;\\ \qquad Index to retrieve $index$;}
$i \gets index$;\\
...
([yshift=1ex]A.east) edge     	node{\small $k_{1},S_1^{-1}$}    	([yshift=1ex]X.west)
([yshift=-1ex]X.west) edge     	node{\small $k_{2}',S_2'$}    	([yshift=-1ex]A.east)
...
(B)    edge[loop right, dashed]    node{\small $k_{m},S_{m}^{-1}$} (B);
...
\noindent\textbf{Using Cascade renovation.} Each of the mixes decrypt and unwrap the records in the first phase then wrap and encrypt in the second, an  blinding phase is added between the two during which the records are only encrypted then decrypted thanks to AES-CTR commutativity as depicted in Fig~\ref{fig:ASMO_CTR}.\\
 %
\begin{figure}[H]
\begin{minipage}{0.475\textwidth}
\begin{adjustbox}{width=\textwidth}
