...
\begin{itemize}
 ...
 \item We give a new security definition of ORAM. %indistiguishability between accessed and unretrieved elements
...
$$\frac{\lambda^*}{1-\lambda^*} \cdot \log\left(\frac{1}{2 \epsilon} \right)\leq \tau(\epsilon) \leq \frac{1}{1-\lambda^*}\cdot \ln\left( \frac{1}{2 \epsilon \cdot \sqrt{p}}\right) $$

We now want to find a bound for $\lambda^*$. We represent the arranging of merge of the 2 distinct sets by $\mathcal{G}$, a $k$-regular bipartite graph with $n$ vertices corresponding to the different arrangings and the undirected edges to transpositions of two elements.
By definition, the eigen value of the transition matrix of the $\mathcal{G}$, $\lambda_0 > \lambda_1 \geq  ... \geq \lambda_{n-1}$, are real positive and we have,

\begin{align*}
\text{diam}\left( \mathcal{G}\right) &\leq \frac{log(n-1)}{log(\frac{k}{\lambda^*})}+1 \text{ with } \lambda^* = max_{i\neq0}(\lambda_i)&\\
log(\frac{k}{\lambda^*}) &\leq\frac{\log(n-1)}{\text{diam}\left(\mathcal{G}\right)-1}&\\
log({\lambda^*}) &\geq log(k) - \frac{log(n-1)}{\text{diam}\left(\mathcal{G}\right)-1}&\\
\lambda^* &\geq \frac{k}{(n-1)^{\frac{1}{\text{diam}\left(\mathcal{G}\right)-1}}}
\end{align*}
