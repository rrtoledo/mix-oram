...
\textbf{Layered method.} In the Layered method, the mixes keep encrypting the records with AES-CBC thanks to IV tokens appended to them.

Before uploading the records to the cloud, the user has encrypted them with AES-CBC and appended to them tokens to generate IVs. During the eviction, the mixes encrypt the records with AES-CBC and these unique IVs, then encrypt the IV tokens thanks to AES-CBC and the mix private key before transmitting the data to the next mixes.
When retrieving a record, before sending the cache back to the ORAM server, the client encrypts the retrieved record with its own key and updates it back in the cache. After doing so, the client locally overwrites the record if executing a write operation or if a read, decrypts the data block in a trial error fashion: the client decrypts first the IV and uses it to decipher the record with the mixes shared secret and repeats these operations $r$ times in total, then the client decrypts the record one more time but with its own key. If the client recognize the plain text, the process stops, else the client cancel his previous decryption and restart the process. In short, the client removes $r$  mix encryption layers with the shared secrets, tries to remove his own with his own keys, checks if the record is in plain text if not cancels its decryption and restarts the process. After reading or writing the record, the client encrypts the record with its own key, and overwrite the cached version with it the next time the cache is fetched. 

With the assumption that the record access distribution is uniform, we can represent the problem of accessing all records as a coupon collector problem. In that case, we expect $E[e]\leq(n/s)\cdot H_n )$ evictions before all records have been fetched once with $H_n$ the n-th harmonic number, and the expected number of layers per card is $E[r]\leq{r/s} \cdot \left ( \frac{n+1}{2}\cdot(H_n-1/2)+1/2 \right )$. For $n=10^6$ and $s=\sqrt{n}$, we have $E[e]\approx 15 \cdot 10^3$ and $E[r]\approx 7\cdot 10^3 \cdot r$.
\begin{proof}
...
To reduce these numbers, we change the access method in this way: when the client requests a record from the database, $d$ other records are chosen uniformly at random from the set of records Database\textbackslash Cache. These records are then fetched, their encryption is refreshed as written previously and the client overwrites with these records their older version on the database. Doing so, choosing a parameter $d$ high enough yields a better approximation of the uniform distribution assumption. We would have thus $E[e]\leq n/(sd) \cdot H_n $ and $E[r] \leq{r/(sd)} \cdot \left [ \frac{n+1}{2}\cdot(H_n-1/2)+1/2 \right ] \ H_n$.
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Encryption key $Keys$; \\ \qquad Record index $i$;\\ \qquad Last records $recs$;\\ \qquad Operation $op$;\\ \qquad Data to write $towrite$; \\ \qquad \# dummies $d$;\\}
$j \gets recover\_index(Keys, i)$;\\
$cache \gets fetch(cache)$;\\
$record \gets decrypt\_cache(Keys,\ i,\ cache)$;\\
\If{$record == null$}
{
$dummies \gets compute_dummies(cache, d+1)$;\\
}
\Else
{
$dummies \gets compute_dummies(cache, d, j)$;\\
}
$d \gets fetch(dummies)$
$push\_cache(recs)$;\\
$send\_cache(cache)$;\\
\If {$op==read$}
{
...
\While{$not\ plain$}
{
	$d[r].data \gets cbc\left( Key, d[r].IV,\ d[r].data \right )$;\\
	$d[r].IV \gets cbc\left(Key, d[r].data,\ d[r].IV \right )$;\\
}
}
$push\_last(encrypt(dummies))$;\\
}
\Else
{
$push\_last(encrypt(towrite, dummies))$;\\
}
\KwOut{data}
\caption{Retrieving a record}
