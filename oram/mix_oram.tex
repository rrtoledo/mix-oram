...
In this work, we use a mix-net for its encryption and permutation functionalities. However, it accepts messages from a single user at a time. We do not seek anonymity, only packet unlinkability.
...
We will consider in this paper two ways to re-encrypt the database, independently of the mix-net architecture, one keeping the number of encryption done by the user constant while the other not.\\
...
\node[draw=none, fill=none]    	(X)				  {};
\node[draw, rectangle, align=center]   		(A)[right of=X]   {\small $\mathbf{U_2}$\\$D_3$};
\node[draw, rectangle, align=center]    		(0)[right of=A]   {\small \textbf{blind} \\ $D_2,\ B$};
\node[draw, rectangle, align=center]    		(B)[right of=0]   {\small $\mathbf{U_1}$\\$D_1,\ R$};
\node[draw, rectangle, align=center]    		(C)[right of=B]   {\small $\mathbf{W_1}$\\$E_1',\ R$};
\node[draw, rectangle, align=center]    		(1)[right of=C]   {\small \textbf{blind}\\ $E_2',\ B$};
\node[draw, rectangle, align=center]    		(D)[right of=1]   {\small $\mathbf{W_2}$\\$E_3'$};
\node[draw=none,fill=none]    	(Y)[right of=D]	  {};

\path
(X.east) edge     	node{\small $ E_{3}E_{2}E_{1}$}    	(A.west)
(A.east) edge     	node{\small $ E_{2}E_{1}$}    	(0.west)
(0.east) edge     	node{\small $B E_{1}$}    	(B.west)
(B.east) edge     	node{\small $ B$}    	(C.west)
(C.east) edge     	node{\small $E_{1}B$}    	(1.west)
(1.east) edge     	node{\small $ E_{2}E_{1}$}    	(D.west)
(D.east) edge     	node{\small $ E_{3}'E_{2}'E_{1}'$}    	(Y.west);
...
([yshift=-1ex]A.west) edge     	node{\small $DB_{k'_{i=1..4m}}^{S'_{j=1..m}}$}    	([yshift=-1ex]0.east)
...
\end{tikzpicture}
\end{adjustbox}
\caption{A simple Mix-ORAM: AES-CBC} \label{fig:ASMO}
\end{figure} 

\begin{figure}
\centering
\begin{tikzpicture}[->, auto, semithick, node distance = 8em]
\tikzstyle{every state}=[fill=white,draw=black,thick,text=black]
...
\node[draw=none, fill=none] (a)[right of=0]{};
\node[draw, rectangle]   		(A')[right=of 0.south, anchor = south]   {\small $Mix_{1}$};

\node[draw=none, fill=none]    		(X)[right of=A]   {\small $...$};
\node[draw=none, fill=none]    		(x)[right of=a]   {};
\node[draw=none, fill=none]    		(X')[right of=A']   {\small $...$};

\node[draw, rectangle]    		(B)[right of= X]   {\small $Mix_m$};
\node[draw=none, fill=none]    		(b)[right of=x]   {};
\node[draw, rectangle]    		(B')[right of= X']   {\small $Mix_m$};

\node[draw, rectangle, minimum height=3cm]    		(T)[right =of b]   {\small $tag$};

\path 
([yshift=0.975cm]0.north east) edge     	node{\small $DB_{k_{i=1..2m}}^{S_j=1..2m}$}    	([yshift=1ex]A.west)

([yshift=1ex]A.east) edge node{$k_{2m},\ S_{2m}^{-1}$}    	([yshift=1ex]X.west)

([yshift=1ex]X.east) edge node{$k_{m+1},\ S_{m+1}^{-1}$}    	([yshift=1ex]B.west)

([yshift=1ex]B.east) edge node{$k_{m},\ S_{m}^{-1}$}     (T.north west)

([yshift=-2ex]T.north west) edge    node{}  ([yshift=-1ex]B.east)

([yshift=-1ex]B.west) edge    node{$k_{m-1},\ S_{m-1}^{-1}$}  ([yshift=-1ex]X.east)

([yshift=-1ex]X.west) edge    node{$k_{2},\ S_{2}^{-1}$}  ([yshift=-1ex]A.east)

([yshift=-1ex]A.west) edge[bend right] node{$k_{1},\ S_{1}^{-1}$} ([yshift=1ex]A'.west)


([yshift=1ex]A'.east) edge node{$k_{1}',\ S_{1}'$}    	([yshift=1ex]X'.west)

([yshift=1ex]X'.east) edge node{$k_{m-1}',\ S_{m-1}'$}    	([yshift=1ex]B'.west)

([yshift=1ex]B'.east) edge node{}     ([yshift=3.5ex]T.south west)

([yshift=1.5ex]T.south west) edge    node{$k_{m}',\ S_{m}'$}  ([yshift=-1ex]B'.east)

([yshift=-1ex]B'.west) edge    node{$k_{m+1}',\ S_{m+1}'$}  ([yshift=-1ex]X'.east)

([yshift=-1ex]X'.west) edge    node{$k_{2m-1}',\ S_{2m-1}'$}  ([yshift=-1ex]A'.east)

([yshift=-1ex]A'.west) edge node{\small $DB_{k_{i=1..2m}'}^{S_{j=1..2m}'}$} ([yshift=-0.8cm]0.south east);


\end{tikzpicture}
\caption{A simple Mix-ORAM: AES-CTR} \label{fig:ASMO}
\end{figure} 
%
