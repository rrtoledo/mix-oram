...
%
We consider here the mix-net in a classic cascade configuration as depicted in Fig~\ref{CascadeArch}. During the eviction process, the whole database is passed from one mix to another such that it is successively processed by all before being updated back to the database.\\
...
The mixes permute the records in the following way.
...
The user when retrieving a given record uses the following algorithm Alg~\ref{alg:CIL} to find the corresponding remote index.
