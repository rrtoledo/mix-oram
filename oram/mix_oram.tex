...
Oblivious RAM (ORAM)~\cite{goldreich87}, or Oblivious Storage (OS)~\cite{boneh2011}, are key technologies to answer such problem. In these designs, the access method is made independent of the input, the sought records, to hide the access pattern. The data-blocks are also periodically randomized to mitigate the information leakage due to data retrieving.
...
Mixing networks is a solution widely applied in Anonymity System to obliviously shuffle packets. A batch of packets is received by the mix-net where it is re-encrypted and shuffled by each mix such that the output of one mix is the next ones' input.
...
In this work, we focus on the eviction algorithm. We present a new security definition for the oblivious sorting algorithm and suggest to leave the eviction to untrusted third parties.
...
Mix-nets were first presented for anonymous e-mailing by David Chaum in 1981~\cite{xxx}, as they became popular other applications such as Tor~\cite{dingledine2004tor} appeared. Mix-net's main goal is to give users some anonymity by hiding the correspondence between the users's packet and. To do so, 
...
The adversaries might have different goals such as identifying a user data access or matching encrypted data-blocks with known plaintext for further purposes such as selective data corruption or monitoring. We consider here the adversary immediate goal as recognizing whether a record was accessed or not. 
...
ORAM designs frequently uses PRGs during the eviction algorithm to shuffle obliviously the records and store the mapping between virtual and real indexes.
...
\end{tikzpicture}
\end{adjustbox}
\caption{A Cascade mix-net} \label{fig:ASMO}
\end{minipage}
\hfill
...
\end{tikzpicture}
\end{adjustbox}
\caption{A Parallel Mixnet} \label{fig:ASMO}
\end{minipage}
\end{figure} 
%
...
Advanced Encryption Standard (AES) was conceived for high speed and low RAM requirements. It can present throughput over 700MB/s per thread on recent CPUs such as the Intel Core i3 or the AMD's models APU and FX which makes it the ideal choice for ORAM block encryption.
Furthermore, AES in Counter mode (AES-CTR) encryption and decryption operations are commutable and commutative. AES-CTR can thus help in the eviction process with re-encrypting the data-blocks with a new key before decrypting the older encryption layer such that the plain text is never revealed. However, as it relies on the plaintext ordering, the practicality of its commutativity is greatly lessened. \\
AES in CBC mode is one of the most common mode of operation used today. It works sequentially, as the ciphertext of one message is needed for the encryption of the next one, and offers speed of similar order to that of CTR.\\ 
...
We will consider in this paper two ways to re-encrypt the database, independently of the mix-net structure. \\
The first way is to decrypt the previous encryption layers and encrypting again, we do so with AES-CTR. Because the records are permuted at every round, the ordering changes and we cannot encrypt before deciphering the old layers. Futhermore, after the unwrapping -and the deciphering- is done, the records are only encrypted with the user encryption and are in an invariant ordering. The adversary looking at successive eviction can spot at this moment which records were accessed by simply observing which user encryption has changed. To prevent such attack, we double the number of unwrapping and wrappings and at the middle of each phase we blind, and respectively unblind, the records as following.
When the first unwrapping is done, the mix(es), besides encrypting the records with AES-CTR and permuting, tag each record with a unique id. The records are then sent in the mix-net organized in cascade where they are locally blinded with blinds generated from the records' tag and mix secrets. After passing through every mix and returning to the original one, the tag is encrypted with El Gamal encryption and the unwrapping resumes. Until the first wrapping ends, the tags' encryption is refreshed thanks to El Gamal additive homomorphism property by adding $E_{EG}(0)$ to the tags. To unblind the records, the mixes broadcast their EG secret share, to reconstruct the EG secret key and decipher the tag, and the records are sent through the mix-net in cascade again so that each mix unblind locally the records. The uncompromised mix blindings are never known by the adversary as the records are encrypted with AES-CTR when being (un)blinded. Thereby, the records after the unwrapping are undistinguishable from random arrays, and the adversary cannot see which records were accessed.
The user when retrieving a record will simply have to decipher the $2*r$ recent AES-CTR layers.\\
The second way to re-encrypt the database consists in keeping encrypting the records with a variant of AES-CBC. The mixnet append to each record a token to generate an IV. The records are encrypted at each round with AES-CBC independently of each other thanks to these unique IVs. Before transfering the records to the next mix, the IV tokens are also encrypted with AES-CBC with an IV generated from the corresponding cipher.
The user when retrieving a record will have to decipher all the AES-CBC encryption layers in a trial \& error fashion : the client will remove $r$ encryption, check if the record is only encrypted with the user encryption if not restart the process. There is $k_i \cdot r$, $k\in \mathbb{N}$, encryption layer for each record $rec_i$. If the record access frequency distribution was close to uniform the $k_i$ would follow a Binomial distribution $\mathbb{B}\left ( e, \frac{s}{n}\right )$ with $e$ the number of evictions done before and be comprised in a relatively small interval. However, in reality, the frequency distribution is not uniform and the $k_i$s are more sparsely distributed. In any case, since the duration of the record decryption can be significantly different, a timing attack can occur. This is why, in this case we advise to change the access method: when a user access a record, he fetches $d$ other records, reencrypt all of the records as previously and rewrite them at the same indices. The number of records fetched before eviction is thus increased to $s\cdot d$, most of which were done randomly. The average and maximum time to decipher is thus reduced, with a large enough $d$ the total deciphering duration is closed to constant so the timing attack mitigated. If the $d$ parameter is not high enough, the client only has to wait the maximum decryption time before rewriting the records.
...
\node[draw, cylinder, shape border rotate=90,minimum height=10ex,minimum width=3ex]    	(X)				  {};
\node[draw, rectangle]   		(A)[right of=X]   {\small $U_1, D_1$};
\node[draw, rectangle]    		(0)[right of=A]   {\small $U', B$};
\node[draw, rectangle]    		(B)[right of=0]   {\small $U_2, D_2$};
\node[draw, rectangle]    		(C)[right of=B]   {\small $W_1, E_1$};
\node[draw, rectangle]    		(1)[right of=C]   {\small $W', B$};
\node[draw, rectangle]    		(D)[right of=1]   {\small $W_2, E_2$};
\node[draw, cylinder, shape border rotate=90,minimum height=10ex,minimum width=3ex]    	(Y)[right of=D]				  {};

\path
(X.east) edge     	node{}    	(A.west)
(A.east) edge     	node{}    	(0.west)
(0.east) edge     	node{}    	(B.west)
(B.east) edge     	node{}    	(C.west)
(C.east) edge     	node{}    	(1.west)
(1.east) edge     	node{}    	(D.west)
(D.east) edge     	node{}    	(Y.west);

\end{tikzpicture}
\end{adjustbox}
\caption{AES-CTR} \label{fig:ASMO}
\end{figure}
...
\node[draw, cylinder, shape border rotate=90,minimum height=10ex,minimum width=3ex]    	(X)				  {};
\node[draw, rectangle]   		(A)[right of=X]   {\small $U, E$};
\node[draw, rectangle]    		(B)[right of=A]   {\small $W, E$};
\node[draw, cylinder, shape border rotate=90,minimum height=10ex,minimum width=3ex]    	(Y)[right of=B]				  {};
...
\node[draw, rectangle, minimum height=10ex]    		(T)[right of=B]   {\small $tag$};
\node[draw, rectangle]    		(B')[right of=T]   {\small $Mix_m$};
...
([yshift=1ex]A.east) edge     	node{\small $k_1,S_1^{-1}$}    	([yshift=1ex]X.west)
([yshift=-1ex]X.west) edge     	node{\small $k_2',S_2'$}    	([yshift=-1ex]A.east)

([yshift=1ex]X.east) edge     	node{\small $k_{m-1},S_{m-1}^{-1}$}    	([yshift=1ex]B.west)
([yshift=-1ex]B.west) edge     	node{\small $k_{m}',S_{m}'$}    	([yshift=-1ex]X.east)

([yshift=1ex]B.east) edge     	node{}    	([yshift=1ex]T.west)
([yshift=-1ex]T.west) edge     	node{\small $k_{m}',S_{m}'$}    	([yshift=-1ex]B.east)

([yshift=1ex]T.east) edge     	node{\small $k_{m},S_{m}^{-1}$}    	([yshift=1ex]B'.west)
([yshift=-1ex]B'.west) edge     	node{}    	([yshift=-1ex]T.east)

([yshift=1ex]B'.east) edge     	node{\small $k_{m+1},S_{m+1}^{-1}$}    	([yshift=1ex]X'.west)
([yshift=-1ex]X'.west) edge     	node{\small $k_{m+1}',S_{m+1}'$}    	([yshift=-1ex]B'.east)

([yshift=1ex]X'.east) edge     	node{\small $k_{2m-1},S_{2m-1}^{-1}$}    	([yshift=1ex]A'.west)
([yshift=-1ex]A'.west) edge     	node{\small $k_{2m}',S_{2m}'$}    	([yshift=-1ex]X'.east)

(A')    edge[loop right, dashed]    node{\small $k_{2m},S_{2m}^{-1}$} (A');
...
\end{tikzpicture}
\caption*{Last rounds of Amortizable sort wrapping} \label{fig:WAS}
\end{minipage}
\hfill
\begin{minipage}[t]{0.45\textwidth}
\begin{tikzpicture}[->,
...
\end{tikzpicture}
\caption*{First rounds of Amortizable sort unwrapping} \label{fig:UAS}
\end{minipage}
