...
When retrieving a record, before sending the cache back to the ORAM server, the client encrypts the retrieved record with its own key and updates it back in the cache. After doing so, the client locally overwrites the record if executing a write operation or if a read, decrypts the data block in a trial error fashion: the client decrypts first the IV and uses it to decipher the record with the mixes shared secret and repeats this operation  $r$ times in total, then the client decrypts the record one more time but with its own key . In short, the client removes $r$  mix encryption layers with the shared secrets, decrypts the record another time with his own key, check if the record is in plain text if not restart the process. After reading or writing the record, the client encrypts the record with its own key, and overwrite the cached version with it the next time the cache is fetched. 
...
\SetKwComment{Comment}{//}{}
\DontPrintSemicolon
\KwIn{Encryption key $Keys$; \\ \qquad Record index $i$;\\ \qquad Last record $rec$;\\ \qquad Operation $op$;\\ \qquad Data to write $towrite$;}
$j \gets recover_index(Keys, i)$;\\
$fetch_cache()$;\\
$decrypt_cache()$;\\
$push_cache(rec)$;\\
\If{$i\ in\ cache$}
