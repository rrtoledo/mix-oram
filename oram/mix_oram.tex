...
To do so, because the symmetric group is not commutative, the mixes first undo the previous permutation (executing $\Pi_{S_m^{-1} \circ .... \circ S_1^{-1}}$) and encrypt with the new encryption keys $k'_i$ before sorting the records (executing $\Pi_{S_1' \circ .... \circ S_m'}$) to the new ordering and decrypting the previous eviction encryption with the $k_i$. We also consider reversing the mix-net flow during the second step, the records now going from the last mix to the first before being uploaded on the database, to avoid unnecessary communication costs and private information transfer between mixes.\\
...
The message format is then : $$E_{pub,\ mix_i}\left(id,\ session_{key},\ \alpha_{i,old},\ \alpha_{i,new},\ (ports,ips)_{signed} \right)$$ 
For 128-bit security, group elements can be expressed in just 32 bytes, we have thus a maximum message size of $|addr|+XXX + 2 \cdot 8 \cdot 32 + XXX$ \todo{}.
...
The adversary could replace the $\alpha_{i+1,new}$ of the innocent mixes with $\alpha_{i+1, old}$ however the user would detect the attack as the index lookup would fail and change of mixes for the next eviction. The user however must check when creating the $\alpha$ that no AES keys and IV are the same between successive evictions.
